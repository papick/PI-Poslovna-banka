...
public class AnalyticsOfStatementService {
	...
	public AnalyticOfStatement saveAnalyticsOfStatementsOrder(File file) throws JAXBException, ParseException {
		...
		if (a.getType().equals("Nalog za uplatu")) {
			...
			DailyAccountState dailyAccountState = dailyAccountStateRepository
					.findOneByDateAndBankAccount(a.getCurrencyDate(), creditorAccount);
	...
	// Bozicu, trebas dodati proveru ako je iznos veci od 250000 da kupis i te
	// naloge i ako su racuni iz razlicitih banaka
			...
			if (dailyAccountStateCreditor == null) {
				...
				if (states.size() != 0) {
					...
					dailyAccountStateNewCreditor.setNewState(dailyAccountStateNewCreditor.getPreviousState()
							+ dailyAccountStateNewCreditor.getPaymentTo()
							- dailyAccountStateNewCreditor.getPaymentFrom());
				...
				dailyAccountStateCreditor.setNewState(dailyAccountStateCreditor.getPreviousState()
						+ dailyAccountStateCreditor.getPaymentTo() - dailyAccountStateCreditor.getPaymentFrom());
	...
	public AnalyticOfStatement saveAnalyticsOfStatementsTransfer(AnalyticOfStatement a)
		...
		if (a.getType().equals("Nalog za prenos")) {
			BankAccount debtorAccount = bankAccountRepository.findOne(a.getDebtorAccount().getId());
			BankAccount creditorAccount = bankAccountRepository.findOne(a.getAccountCreditor().getId());
			if (debtorAccount.getBank().getId() == creditorAccount.getBank().getId()) {
				DailyAccountState dailyAccountState = dailyAccountStateRepository
						.findOneByDateAndBankAccount(a.getDateOfReceipt(), debtorAccount);
				DailyAccountState dailyAccountStateCreditor = dailyAccountStateRepository
						.findOneByDateAndBankAccount(a.getDateOfReceipt(), creditorAccount);
				...
				if (dailyAccountState == null) {
					ArrayList<DailyAccountState> states = dailyAccountStateRepository
							.findAllByBankAccount(debtorAccount);
					// ako ne postoji ni jedno dnevno stanje za dati racun ,
					// nije
					// moguce izvrstiti prenos jer je stanje na rucnu 0
					if (states == null) {
						throw new IllegalArgumentException("Ne posotoji dovoljno novca za naplatu !");
				...
				if (dailyAccountStateCreditor == null) {
					ArrayList<DailyAccountState> states = dailyAccountStateRepository
							.findAllByBankAccount(creditorAccount);
					// ako postoje dnevna stanja za dati racun
					// pronaci najveci datum dnevnog stanja racuna
					if (states.size() != 0) {
						DailyAccountState max = states.get(0);
						...
						DailyAccountState dailyAccountStateNewCreditor = new DailyAccountState();
						// setovanje dailyState za poverioca kad ne postoji
						// nijedno
						// dnevno stanje za njega
						dailyAccountStateNewCreditor.setBankAccount(creditorAccount);
						dailyAccountStateNewCreditor.setPreviousState(0.0);
						...
						dailyAccountStateNewCreditor
								.setPaymentTo(dailyAccountStateNewCreditor.getPaymentTo() + a.getSum());
	...
	public void generateBankTransfer(AnalyticOfStatement a) throws JAXBException {
		...
		if (a.getSum() < maxSum && !a.getEmergency()) {
			...
			if (clearing == null) {
				...
				analytics.add(a);
				clearing = new Clearing(fromBank, toBank, a.getPaymentCurrency(), a.getCurrencyDate(), analytics,
						a.getSum());
			} else {
